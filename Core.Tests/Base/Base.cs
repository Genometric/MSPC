...
namespace Genometric.MSPC.Core.Tests.Base
{
    public class Base
    {
        ...
        public void DiscardAll()
        {
            ...
            var bed2 = new GeUtilities.IntervalParsers.BED<ChIPSeqPeak>();
            bed2.Add(new ChIPSeqPeak() { Left = 30, Right = 40, Value = 1e-8, Name = "2" }, "chr1", '*');

            var mspc = new MSPC<ChIPSeqPeak>();
            mspc.AddSample(1, bed1);
            mspc.AddSample(2, bed2);

            var config = new Model.Config(Model.ReplicateType.Biological, 1e-8, 1e-8, 1e-8, 2, 0.05F, Model.MultipleIntersections.UseLowestPValue);
