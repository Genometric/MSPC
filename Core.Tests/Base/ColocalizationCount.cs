...
namespace Core.Tests.Base
{
    public class ColocalizationCount
    {
        ...
        public void SingleNonOverlappingPeak(byte c, byte expected)
        {
            ...
            // Assert
            Assert.True(new[] { res[0].Chromosomes["chr1"].Get(Attributes.Output).Count, res[1].Chromosomes["chr1"].Get(Attributes.Output).Count }.All(x => x == expected));
        }
        ...
        public void ThreePeaksTwoOverlapping(byte c, byte expected)
        {
            ...
            // Assert
            Assert.True(new[] { res[0].Chromosomes["chr1"].Get(Attributes.Output).Count, res[1].Chromosomes["chr1"].Get(Attributes.Output).Count, res[2].Chromosomes["chr1"].Get(Attributes.Output).Count }.All(x => x == expected));
        }
        ...
        public void OnlyOnePeakPerSampleIsConsideredForC()
        {
            ...
            // Assert
            Assert.True(res[0].Chromosomes["chr1"].Get(Attributes.Output).Count == 0);
        }
    }
}
