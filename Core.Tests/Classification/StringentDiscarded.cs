...
namespace Core.Tests.Base
{
    ...
    {
        private ReadOnlyDictionary<uint, AnalysisResult<ChIPSeqPeak>> CreateStringentPeaksAndDiscardThem()
        {
        ...
        public void Separate2()
        {
            ...
            foreach (var s in res)
                foreach (var p in s.Value.R_j__c["chr1"])
                    Assert.True(p.Value.classification == PeakClassificationType.StringentDiscarded);
        }
    }
}
