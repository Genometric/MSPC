...
namespace Core.Tests.Base
{
    ...
    {
        private ReadOnlyDictionary<uint, AnalysisResult<ChIPSeqPeak>> CreateWeakPeaksAndConfirmThem()
        {
        ...
        public void Separate()
        {
            ...
            foreach (var s in res)
                Assert.True(s.Value.Stats("chr1", PeakClassificationType.WeakConfirmed) == 1);
        }
        ...
        public void Separate2()
        {
            ...
            foreach (var s in res)
                foreach (var p in s.Value.R_j__c["chr1"])
                    Assert.True(p.Value.classification == PeakClassificationType.WeakConfirmed);
        }
    }
}
