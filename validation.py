...
def coverageDifference(pA, nA, pB, nB):
	try:
		b = pA - pB
...
'''
E = pandas.read_csv('Multiple_Peak_Calling_applicationNote/MSPC_docs/MSPC_comparison_results.txt', sep = "\t", header = 0)

#def boxplot(data):
#box = os.path.join(data.split('.')[0], '_boxplot.pdf')
#E = pandas.read_csv(data, sep = "\t", header = 0)
E = E[E.N_peaks >= 20]     # Enriched peaks threshold
#E['dataset'] = E.dataset.astype('category')
#E['dataset'] = pandas.factorize(E.dataset)[0]
#E['color'] = E['dataset'].map({'common': 'deepskyblue2',
#                               'specific': 'gold',
#                               'missing': 'green3'})
ggplot(E, aes(x = 'dataset', y = 'z', fill = 'dataset')) + geom_boxplot()
'''
...
if len(sys.argv) == 1:
	...
	if DIM > 1:
		...
		oth = open(otf, 'w')
		oth.write('annotation\tN_anno\tdataset\tN_peaks\t' +\
	...
	for root in DATA:
		...
		ran = open(res, 'w')
		ran.write('annotation\tN_anno\tdataset\tN_peaks\t' +\
		          'estimate\tCI95\tz\tSE\tpvalue\n')
		for annotation in absPaths(MspcFE_conf['annotationDirectory']):
			...
			for x, y in zip((cpf, spf, mpf), U):
				...
				#print A[tag]
				if (Q[0] == 1):
					line = tag + '\t' + str(MspcFE_conf[tag][0]) +\
		...
		# !! NOT WORKING !! #
		'''
		call('Rscript MSPC_FunctionalEnrichment/' +\
		     'R/enrichment_plots.R ' + res + ' boxplot',
		     shell = True)
	...
	if DIM > 1:
		...
		'''
		call('Rscript MSPC_FunctionalEnrichment/' +\
		     'R/enrichment_plots.R ' + otf + ' boxplot',
		     shell = True)
		'''
